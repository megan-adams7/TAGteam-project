...
c <- 1
for (n in x) {
  m0 <- read.table(file = paste("all_mlp1/500_upstream/plus/canonical_tagteam/csv/", n, "_genes_plus_500_upstream.csv", sep= ""), sep = '\t', header = TRUE)
...
#minus
for (n in x) {
  m0 <- read.table(file = paste("all_atonal/500_upstream/minus/canonical_tagteam/csv/", n, "_genes_minus_500_upstream.csv", sep= ""), sep = '\t', header = TRUE)
  m0$seqID[0:length(m0$seqID)] <- n
  colnames(m0)[1] <- "seq_id"
...
l0 <- read.csv("/workdir/maa367/table.csv")
...
q <- gggenomes(big_data, l0) +
  ...
  ggtitle("atonal")
ggsave("atonal_plot.png", q, width = 12, height = 30, dpi = 300)
```
